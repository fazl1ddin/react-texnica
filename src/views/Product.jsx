...
function Product({ user, setLoginModal }) {
  ...
  const descriptions = ["Описание", "Характеристики", "Отзывы"];
  ...
  const [comments, cLoading, refetch] = useGetComments(
    "/comments",
    products._id
  );
              ...
              <form>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    await fetch(config.baseUrl + "/add-comment", {
                      method: "POST",
                      body: JSON.stringify({
            ...
            <div className="productGallery">
              <div className="central">
                {products.product.map((item, i) => (
                  <div key={i}>
                    <img src={`${item}`} className="productImg" alt=""/>
                    {item.protection ? (
                      <img
              ...
              <div className="galleryPagination">
                ...
                <div className="galleryPaginationItems">
                  {pagination().map((item, i) => (
                    <div
                      className={`galleryPaginationItem ${
                        current === item ? "active" : ""
                      }`}
                      onClick={() => setCurrent(item)}
                      key={i}
                    >
                      <img src={products.product[item]} alt=""/>
                    </div>
                  ))}
                </div>
                <button
                  disabled={current >= products.product.length - 1}
                  onClick={() =>
                    setCurrent(
                      Math.min(current + 1, products.product.src.length - 1)
                    )
                  }
                >
                  &#9205;
              ...
              <div className="productPrices">
                <div className="productPricesBox">
                  <div className="rateStar">
                    <div className="ratesStars">{stars(products.rates)}</div>
                    <div className="comments">
                      <img src={img.messageSquare} alt=""/>
                      <h5>({cLoading ? "saffsaf" : comments.length})</h5>
                    </div>
                  </div>
                ...
                <div className="productPricesBox">
                  <div className="pricesText">
                    <div className="fflex">
                      <del
                        className={`${
                          products.price === products.realPrice ? "visible" : ""
                        }`}
                      >
                        {products.price}₽
                      </del>
                      <h4>
                        <span className="spanone">{products.sale} %</span>{" "}
                        <span className="spantwo">— {products.space} ₽</span>
                      </h4>
                    </div>
            ...
            <div className="desNav">
              <ul>
                {descriptions.map((item, i) => (
                  <li
                    key={item}
                    className={currentDes === i ? "active" : ""}
                    onClick={() => setCurrentDes(i)}
                  >
                    {item === "Отзывы"
                      ? `${item} (${cLoading ? "asffsafas" : comments.length})`
                      : item}
                  </li>
                ))}
              </ul>
            ...
            <div className="tabs">
              ...
              {currentDes === 0 ? (
                <div className="description">
                  <p
                    ...
                    dangerouslySetInnerHTML={{
                      __html: products.description.content,
                    }}
                  ></p>
                  <h1>Рекомендуем</h1>
                ...
                <div className="specification">
                  <table>
                    <tbody>
                      {Object.keys(products.specification)
                        .splice(2)
                        .map((item, i) => (
                          <tr key={item}>
                            <th>{item}</th>
                ...
                <div className="rates">
                  ...
                  {user ? (
                    ...
                    <div className="newRate">
                      ...
                      <button onClick={() => setLoginModal("login")}>
                        Написать отзыв
                      </button>
                    </div>
                  )}
