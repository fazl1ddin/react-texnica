...
function Favorites({ setModal, user, loadingUser, drops }) {
  ...
  const products = useCallback(() => {
    ...
    if (settings.filter != "Все" && settings.filter != "")
      allProducts = allProducts.filter(
        (item) => item.specification["Тип:"] == settings.filter
      );
    if (settings.filterPrice != "")
      allProducts.sort((a, b) =>
        settings.filterPrice == "expensive"
          ? b.realPrice - a.realPrice
          : a.realPrice - b.realPrice
      );
    return allProducts;
  }, [settings, data]);
        ...
        <div className="favoritesContent">
                ...
                <ul>
                  {drops.map((item) => (
                    <li key={item.title}>
                      <Link
                        className={
                          item.path === "/favorites" ? "activeDropP" : ""
                        }
                        to={
                          item.stateTab
          ...
          <div className="filterContent">
            <div className="filter">
              <select
                onChange={(e) =>
                  setSettings({ ...settings, filter: e.target.value })
                }
              >
              ...
              <select
                onChange={(e) =>
                  setSettings({ ...settings, filterPrice: e.target.value })
                }
              >
                  ...
                  <div className="xityProdajBox mb-15" key={every._id}>
                    <Link to={`/product/${every._id}`}>
                      <div className="sigveiWrap">
                        <img
                          onClick={() =>
                            updateOne(
                              some("viewed", every._id) ? "remove" : "add",
                              "viewed",
                              every._id
                            )
                          }
                    ...
                    <div className="prices">
                      <div className="pricesText">
                        <del
                          className={`${
                            every.price == every.realPrice ? "visible" : ""
                          }`}
                        >
                          {every.price} ₽
                        </del>
