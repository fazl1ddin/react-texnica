    ...
    switch (props.type) {
        case 'phone':
            result = <div data-valid={props.valid} className="modalWrapper" key={props.name}>
                <label htmlFor={props.name}>{props.title}</label>
                <input type={props.type} placeholder={props.placeholder} value={props.value} id={props.name} onKeyDown={(e) => props.keyDown(props, e)} onChange={(e) => props.change(props, e)} onFocus={(e) => props.focus(props, e)} onBlur={(e) => props.blur(props, e)}/>
                {
                    props.valid === 0 ? null : props.valid === 1 ?
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                    <g id="icon-right" clipPath="url(#clip0_179_8163)">
